...
Even though proportions of cut types are not close to each other, using log(price) instead of price is more preferable since price has normal distribution in each category. 

```{r price-cut, message=FALSE}
# Using price
ggplot(raw_diamonds) + geom_histogram(aes(x = price)) + facet_wrap(~cut)

# Using log(price)
ggplot(diamonds) + geom_histogram(aes(x = log.price)) + facet_wrap(~cut)
```
